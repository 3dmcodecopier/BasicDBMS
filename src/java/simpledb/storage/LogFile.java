    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                ...
                long beginOffset = tidToFirstLogRecord.get(tidId); // tidToFirstLogRecord: when begin a record, put a tid and corresponding offset
                raf.seek(beginOffset);
                while (true) {
                    int cpType = raf.readInt();
                    long cpTid = raf.readLong();
                    if (cpType == 3) {
                        if (cpTid == tidId) { // because mutiple transactions are excuting, cpTid need verification
                            Page before = readPageData(raf);
                            Page after = readPageData(raf);
                    ...
                    raf.seek(raf.getFilePointer() + 8);// skip currentoffset(8) to the next record
                }
            }
        }
    }
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                // some code goes here
                //Analysis Phase
                Map<Long, Long> newTidOffset = new HashMap<>();
                Map<PageId, Page> pages = new HashMap<>();
                Map<Long, ArrayList<PageId>> pageIds = new HashMap<>();
                ...
                if (startOffset != -1L) {
                    ...
                    //The DBMS periodically takes checkpoints where it writes the dirty pages in its buffer pool out to disk. This
                    //is used to minimize how much of the log it has to replay upon recovery.
                    if (logRecord instanceof CheckPointRecord) newTidOffset = ((CheckPointRecord) logRecord).getTidToFirstLogRecord();
                    else
                        throw new IOException("checkpoint at wrong place");
                }
                //to identify dirty pages in the buffer pool and active transactions at the time of the crash
                for (Long tid : newTidOffset.keySet()) {
                        tidToFirstLogRecord.put(tid, newTidOffset.get(tid));//I guess it's the ATT in ARIES recovery
                    if (!pageIds.containsKey(tid)) pageIds.put(tid, new ArrayList<>());//DPT
                }
                //Redo Phase
                while (raf.getFilePointer() < raf.length()) {
                    LogRecord newRecord = LogRecord.readNext(raf);
                    if (null == newRecord) break;
                    //update: store page in cache(pages, pageIds)
                    ...
                    } //abort: remove pafe from cache
                    else if (newRecord instanceof AbortRecord) {
                        long tid = newRecord.getTid();
                        tidToFirstLogRecord.remove(tid);
                        for (PageId pid : pageIds.get(tid)) {
                            pages.remove(pid);
                        }
                        pageIds.remove(tid);
                    } //begin: write into tidToFirstLogRecord and pageIds
                    else if (newRecord instanceof BeginRecord) {
                        long tid = newRecord.getTid();
                        tidToFirstLogRecord.put(tid, newRecord.getOffset());
                        pageIds.put(tid, new ArrayList<PageId>());
                    } //commit: write page into disk
                    else if (newRecord instanceof CommitRecord) {
                        long tid = newRecord.getTid();
                        tidToFirstLogRecord.remove(tid);
                        for (PageId pid : pageIds.get(tid)) {
                ...
                //Undo phase: add ABORT_RECORD, rollback(especially for update_record)
                currentOffset = raf.getFilePointer();
